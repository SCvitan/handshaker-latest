...
interface DocumentationStepProps {
  data: Documentation
  onUpdate: (data: Documentation) => void
  onBack: () => void
  onSaveAndHome: () => void
  onFinish: () => void
  isSaving: boolean
...
export function DocumentationStep({
  ...
  const handleRemove = async (doc: DocumentItem) => {
    try {
      ...
      onUpdate(data.filter((d) => d.id !== doc.id))
    } catch (err) {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Upload your important documents. These help verify your profile and speed up the hiring process.
        </p>
      </div>
